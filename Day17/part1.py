...
def find_max_y(x_target, y_target):
    max_y = 0
    for x_velocity in range(x_target[1]):
        for y_velocity in range(abs(y_target[0])):
            simulation_result = perform_simulation(x_velocity, y_velocity, x_target, y_target)
            if simulation_result:
                max_y = max(max_y, simulation_result)
...
result = find_max_y((min_x, max_x), (min_y, max_y))
