...
def extract_scanner_beacons_into_main_scanner(extracted_scanner):
    ...
    if len(intersection) >= NB_MATCHING_VECTORS:
        ...
        SCANNERS_COORDINATES.add(origins_vector_from_a)
...
max_manhattan = 0
for first_scanner, second_scanner in itertools.combinations(SCANNERS_COORDINATES, 2):
    manhattan = sum(abs(first_scanner[i] - second_scanner[i]) for i in range(3))
    max_manhattan = max(max_manhattan, manhattan)

print("Result:", max_manhattan)
